...
const consentBody=z.object({displayName:z.string().min(1).max(120).optional(),termsVersion:z.string().min(1).max(50),privacyVersion:z.string().min(1).max(50)});
...
export async function registerAuthRoutes(app:FastifyInstance,pool:DbPool){
  app.post('/v1/auth/sync',async(request,reply)=>{
    ...
    const {rows}=await pool.query(`insert into members(id,auth_subject,email,display_name,account_status,onboarding_status,is_admin,terms_version,terms_accepted_at,privacy_version,privacy_accepted_at)
      values($1,$1,$2,$3,'active','not_started',$4,$5,now(),$6,now())
      on conflict(id) do update set email=excluded.email,display_name=coalesce(excluded.display_name,members.display_name),terms_version=excluded.terms_version,terms_accepted_at=excluded.terms_accepted_at,privacy_version=excluded.privacy_version,privacy_accepted_at=excluded.privacy_accepted_at,updated_at=now()
      returning id,email,display_name,account_status,onboarding_status,is_admin,created_at`,[request.betAuth.userId,email,body.displayName??null,request.betAuth.role==='admin',body.termsVersion,body.privacyVersion]);
